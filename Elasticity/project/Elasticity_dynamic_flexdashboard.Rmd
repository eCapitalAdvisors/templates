...
dateRangeInput(inputId = "Date_Range",
          label = h4("Date Selection:"),
          start = 1989-09-14,
          end = 1997-05-14,
          min = min(sales_tbl$start),
          max = max(sales_tbl$end),
          format = "yyyy-mm-dd",
          startview = "month",
          weekstart = 4)
```
...
observeEvent(eventExpr = input$reset, handlerExpr = {
  ...
  updateDateInput(session = session,
                  inputId = "Date_Range",
                  start = min(sales_tbl$start),
                  end = max(sales_tbl$end))
})
```
...
#  sales_tbl %>%
#  filter(sales, quantity, price) %>%
#  mutate(revenue = price * sales / quantity) %>%
#  filter(revenue )
#})
