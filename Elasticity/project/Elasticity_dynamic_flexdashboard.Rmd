...
total_dataset <- reactive({
  sales_tbl %>%
    group_by(start_year) %>%
    mutate(start = as.Date(start), end = as.Date(end), total_revenue = sum(revenue)) %>%
    filter(description %in% input$Cereal_brand,
           city %in% input$Store_location,
           start_year %in% input$Year_choosen)
})

# why are start, end, and start_year decimals?
...
output$line_revenue_plot <- renderPlotly({
  plot_total_revenue_line(total_dataset(), start_year, total_revenue, description)
})
```
